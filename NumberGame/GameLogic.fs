...
module Logic = 
    ...
    let boundsFrom (consoleInput: string[]) =
    ...
    let answerBetween (bounds: Bounds) : Answer = 
        ...
        | (lower, upper) -> 
            answerFor ((System.Random()).Next(lower, upper))

    let checkGuessFor (r: Round) = 
    ...
    let resultStringFor (result: RoundResult) = 
        match result with 
    ...
    let rec guessNumber (correctAnswer: Answer) (bounds: Bounds) (statusMessage: string) =
        ...
        | Some value  -> 
            match checkGuessFor (guessFor value, correctAnswer, bounds) with 
